...
def handle_get_me(event):
    ...
    user["guardians"] = guardians_for_user(user_id)
...
def handle_get_guardians(event):
    ...
    if err:
        return err
    return response(200, {"guardians": guardians_for_user(user_id)})
...
def handle_create_guardian(event):
    ...
    guardian_email = body.get("guardianEmail") or body.get("email") or ""
    ...
    return response(201, {"guardian": item})
